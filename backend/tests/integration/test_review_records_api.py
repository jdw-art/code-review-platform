...
from app.db.models import Project, ProjectTemplate
...
def test_review_records_api_supports_mock_ingest_list_detail_and_raw(
    ...
    assert isinstance(ingested["id"], int)
    assert ingested["project_id"] == project.id
    assert ingested["project_key"] == project.key
    assert ingested["commit_count"] == 2
    ...
    listed = next(item for item in list_body["items"] if item["id"] == ingested["id"])
    ...
    detail_response = authenticated_superuser_client.get(
        f"/api/v1/review-records/{ingested['id']}"
    )
    assert detail_response.status_code == 200
    detail = detail_response.json()
    assert detail["id"] == ingested["id"]
    ...
    raw_response = authenticated_superuser_client.get(
        f"/api/v1/review-records/{ingested['id']}/raw"
    )
    assert raw_response.status_code == 200
    raw = raw_response.json()
    assert raw["id"] == ingested["id"]
